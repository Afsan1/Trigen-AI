...
import { VideoIcon, Download, ArrowLeft, Loader2, Sparkles } from 'lucide-react';
            ...
            <div className="w-full max-w-7xl">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white flex items-center gap-3">
                        <VideoIcon className="text-blue-500" /> {isSingleView ? "Generated Video" : "My Video Library"}
                    </h1>
                </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2">
                                <div className="rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-2xl relative group bg-slate-100 dark:bg-black flex items-center justify-center min-h-[400px]">
                                    ...
                                    <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-sm">
                                        <a 
                                            href={singleUrl} 
                                            download={`VidMaxx_Video_${Date.now()}.mp4`}
                                            target="_blank" rel="noreferrer"
                                            className="bg-white/90 text-slate-900 px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:scale-105 transition shadow-xl text-sm"
                                        >
                            ...
                            <div className="space-y-6">
                                ...
                                <button onClick={() => navigate('/video-library')} className="w-full bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-3 rounded-xl font-bold hover:opacity-90 transition">
                                    View Full Library
                                </button>
                            </div>
                        </div>
                        ...
                        <>
                            {isLoading ? (
                                <div className="flex flex-col items-center justify-center h-[400px]">
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 font-medium">Loading your videos...</p>
                                </div>
                            ) : fetchError ? (
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center py-20">
                                    <div className="p-4 bg-blue-100 dark:bg-blue-900/30 rounded-full mb-4">
                                        <VideoIcon size={40} className="text-blue-400" />
                                    </div>
                                    <h2 className="text-2xl font-semibold mb-2 text-slate-800 dark:text-white">No videos found!</h2>
                                    <p className="text-slate-500 dark:text-slate-400 max-w-md mb-6">
                                        Your library is currently empty. Head over to the creator to generate your first video!
                                    </p>
                                    <button 
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {videos.map((vid) => (
                                        <div key={vid.id} className="group bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-lg hover:shadow-xl hover:-translate-y-1 transition duration-300 flex flex-col">
                                            <div className="relative aspect-video overflow-hidden bg-slate-200 dark:bg-black">
                                                <video 
                                                    src={vid.url} 
                                                    controls
                                                    ...
                                                    preload="metadata"
                                                ></video>
                                                <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition duration-300 flex items-center justify-center backdrop-blur-sm pointer-events-none group-hover:pointer-events-auto">
                                                    <a 
                                                        href={vid.url} 
                                                        download={`VidMaxx_${vid.id}.mp4`}
                                                        target="_blank" rel="noreferrer"
                                                        className="bg-white/90 text-slate-900 px-4 py-2 rounded-lg font-bold flex items-center gap-2 shadow-xl hover:scale-105 transition"
                                                    >
                                                        <Download size={16} /> Download
                                                    </a>
                                                </div>
                                            ...
                                            <div className="p-5 flex flex-col flex-1">
                                                <h3 className="font-bold text-lg text-slate-900 dark:text-white mb-2 line-clamp-1">{vid.title || "Untitled"}</h3>
